...
void PLL_160MHz(void)
{
  ...
  */
   ...
   * 10/(2*5) = 10/10 = 1, so same frequency as PLL0
   */
  PLLDIG.PLL1DV.B.RFDPHI = 10;
  PLLDIG.PLL1DV.B.MFD = 5;
  ...
  /* Configure RUN_PC for LINFlexD_1 */
  MC_ME.RUN_PC[1].R = 0x000000FE; //Configure RUN_PC[1] for all modes
